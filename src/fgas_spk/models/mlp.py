"""Plain feed-forward MLP regressor for f_gas(R) to SP(k) (PyTorch).

A deliberately simple deep neural network: a single feed-forward stack maps the
f_gas(R) profile (optionally augmented with conditioning context) straight to the
SP(k) target, with **no** bottleneck. It is the un-bottlenecked sibling of
:mod:`~fgas_spk.models.mlp_regressor` -- the encoder/narrow-latent/decoder split
of that model exists to seed the later CVAE, whereas this plugin is a plain
regressor with nothing in the middle. Everything else about the two is the same:
the standardisation, the conditioning wiring, the determinism contract, and the
per-epoch history.
...
scale and the two are concatenated into a single context vector, which is
concatenated to the profile at the network input exactly as
:mod:`~fgas_spk.models.pca_linear` concatenates ``X_cond``. With no conditioning
the network reads the profile alone. Both modalities are wired in identically, so
``X_params`` is used exactly as ``X_cond`` is.

Because there is no bottleneck, this model exposes **no** ``latents`` method (there
is no code to inspect) and **no** ``predict_samples`` (it is a deterministic point
predictor). The runner's duck-typed latent-norm and probabilistic-coverage
diagnostics are therefore skipped for it -- correctly, since neither is defined.
...
(:mod:`fgas_spk.train`) threads it into ``metrics.jsonl`` and the training-history
figure when present.

In a run, the model is constructed from the
:class:`~fgas_spk.experiment.RunConfig` as
``Mlp(seed=run_config.seed, **run_config.model_params)`` and looked up by name via
``fgas_spk.models.REGISTRY["mlp"]``.
...
@register("mlp")
class Mlp:
    """Plain feed-forward MLP mapping f_gas(R) (+ conditioning) to SP(k).

    The model standardises its inputs and target internally (see the module
    docstring) and trains a single ``in -> hidden... -> n_k`` network with Adam
    ...
    :meth:`fit` batch; nothing about the data dimensions is hardcoded. There is no
    bottleneck -- this is the un-bottlenecked sibling of
    :class:`~fgas_spk.models.mlp_regressor.MlpRegressor`.

    Args:
        hidden (int): Hidden-layer width. Defaults to 256.
        n_layers (int): Number of hidden layers in the stack. Defaults to 2.
    ...
    Attributes:
        history (list[dict]): One ``{"epoch", "train_mse"}`` dict per epoch,
            populated by :meth:`fit`. The runner records it into ``metrics.jsonl``
            and the training-history figure.
    """
    ...
    def __init__(
        ...
        **_: object,
        ...
        # The network is built (and torch imported) in fit, once dims are known.
        self.hidden = hidden
    ...
    def fit(self, training_data: "TrainingData") -> None:
        """Fit the feed-forward network on a TrainingData bundle.
        ...
        concatenated into one context vector fed to the network input; whichever
        ...
        if self._uses_cond:
            print('Using conditioning scalars (X_cond) in Mlp.fit.')
        ...
        # Seed, resolve the device, and build the network now that dims are known.
        torch.manual_seed(self.seed)
        self._device = self._resolve_device()
        self._build_net(n_profile, n_context, n_k)
        ...
        for epoch in range(self.epochs):
            ...
            self._net.eval()
            with torch.no_grad():
                pred_all = self._forward(Xs, conds)
    ...
    def predict(
        ...
        if self._net is None:
            raise RuntimeError("Mlp.predict called before fit.")
    ...
    def _build_net(self, n_profile: int, n_context: int, n_k: int) -> None:
        """Build the feed-forward network (torch imported lazily here).

        ``n_context`` is the combined width of the conditioning modalities
        (``X_cond`` plus ``X_params``); it widens the network input.
        """
        self._net = self._mlp(n_profile + n_context, n_k)
    ...
    def _forward(self, x, cond):
        """Run ``concat(x, cond)`` (or ``x`` alone) through the network."""
        import torch

        net_in = x if cond is None else torch.cat([x, cond], dim=1)
        assert self._net is not None
        return self._net(net_in)
    ...
    def _resolve_device(self):
        ...
        if self.device is not None:
            print(f"Using user-specified device '{self.device}' for Mlp.")
            return torch.device(self.device)
        if torch.cuda.is_available():
            print("Using CUDA device for Mlp.")
            return torch.device("cuda")
        if torch.backends.mps.is_available():
            print("Using MPS device for Mlp.")
            return torch.device("mps")
        print("Using CPU device for Mlp.")
    ...
    def _context(
        ...
        with its stored statistics) fed to the network input, or None if neither
    ...
    def _check_modality(
        ...
        if self._uses_cond and X_cond is None:
            raise ValueError(
                "This model was fit with conditioning (X_cond); predict "
        ...
        if self._uses_params and X_params is None:
            raise ValueError(
                ...
                "predict requires X_params as well."
            )
